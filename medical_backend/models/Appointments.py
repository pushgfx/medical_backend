...
class Appointments:
    ...
    def add_appointment(self,request,patient_id):
        ...
        params=( str(req_patient_id), str(req_office_id), str(req_doctor_id), str(req_was_reffered),
        str(req_was_reffered_by), str(req_appt_start_time), str(req_appt_end_time), str(req_appt_status),
        str(req_appt_booking_date), str(req_appt_booking_method), str(req_reason_for_visit)
        )
